...
function ht_en = run_controller(T_target, T_home, t)
  ...
  global last_t = 0;
  global window_start_time = 0;
  global on_time = 0;
  
  MIN_ON = 1*60;
  WINDOW_SIZE = 2*60; % Lower values = lower overshoot
  Kp = 1.1; % Reduce to reduce overshoot
  Ki = 1e-4;

  error = (T_target - T_home);
  dt = (t - last_t);
  integral += error*dt;
  ...
  output = Kp*error + Ki*integral;% + 1e2*derivative;
  output = min(1, output);
  output = max(0, output);
  
  if (t - window_start_time >= WINDOW_SIZE)
    window_start_time += WINDOW_SIZE;
    on_time = WINDOW_SIZE * output; % compute new
  ...
  ht_en = 1;
  if (on_time <= t - window_start_time)
    ht_en = 0;
  endif
end
