...
function ht_en = run_controller(T_target, T_home, t)
  global histeresis = 0; % keep a global state
  
  trig_delta = 0.5; % delta for trigger operation
  overshoot_factor = 0.7; % from 0 to 1 compensates for overshoot
  
  ht_en = 1;
  
  if (T_home > T_target - (histeresis*trig_delta))
    ht_en = 0;
    histeresis = 1;
  elseif (T_home <= T_target - (histeresis*trig_delta))
    ht_en = 1;
    histeresis = overshoot_factor; % due to overshoot this works better
  else % in between keep doing what we were doing before
    ht_en = not(histeresis); % true only if equal to one
  endif
end
...
for i = 1:(T_SIM_MINS*60/T_SAMPLE)
  ht_en = run_controller(si(t0), x0(2), t0);
